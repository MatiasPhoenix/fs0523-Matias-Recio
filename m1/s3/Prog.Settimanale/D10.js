...
function add (){
  return me.skills = ["HTML", "CSS", "JavasCript"];
}
...
//     valid = false;
//     console.log("questa non è una mail");
//   }
//   return console.log("bravo, questa è una mail");;
// }
// isThisAnEmail("asdasdasd@gmail.com");
...
function movieLenght (array){
  let newArray = array;
  let risultato = newArray.length;
  return risultato;
}
...
function onlyTheYears (array){
  let provArray = array;
  let newArray = provArray.map((elem) => elem.Year)
  return newArray;
}
...
function sumAllTheYears(array){
  let provArray = array;
  let newArray = provArray.map((movies) => parseInt(movies.Year))
  let sum = 0;  
...
function searchTag(){
  let selection = document.querySelectorAll("td");
}
